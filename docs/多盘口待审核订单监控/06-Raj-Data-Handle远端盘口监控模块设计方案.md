...
配置遵循“系统设置管理 Telegram 目的地和模板；远端盘口监控管理全局运行参数、每盘口及每指标策略”的边界。两个页面如展示同一字段，必须读写同一条配置记录，不能复制两份状态。
...
| `bot_token` | 管理员直接填写；提交后加密保存且不回显 |
| `chat_id` | 管理员直接填写；提交后加密保存且不回显 |
| `template_set_id` | 默认使用的模板集 |
| `parse_mode` | 首版固定为安全转义后的 HTML，禁用网页预览 |

Bot Token 和 Chat ID 使用系统统一凭据密钥加密后存入数据库；接口响应、日志和审计只返回 `configured: true/false`，不回显明文。原有 `env://` 引用继续兼容。测试消息由管理员在远端盘口监控页选择目的地和盘口后显式触发，写操作审计，但不改变真实事件和冷却时间。
...
| `monitor_notification_destinations` | 目的地、加密 Telegram 凭据及兼容的旧密钥引用 |
...
2. 全局运行参数：页面刷新、远端查询默认值、重复告警间隔、连续次数和投递模式；
3. Telegram 测试：管理员选择通知目的地及盘口，发送带所选盘口名称的测试消息；
4. 盘口卡片：待审核、待审查、各自阈值与事件状态、最后成功时间、下次检查、最近安全错误；
5. 管理员配置抽屉：盘口级和指标级字段；
6. “立即只读检查”操作；若需重新登录或测试连接，跳转到“远端账号”，不在监控页复制凭据操作。

系统设置增加“Telegram 通知目的地”和“通知模板”区块。普通业务用户可查看监控状态，不可修改配置、测试目的地或查看凭据细节；现有角色模型下写操作统一要求 `admin`。
...
- 完成加密凭据、兼容密钥引用、目的地、模板、Outbox、重试和审计；
- 完成新 API 和页面配置抽屉；
