...
> 状态：P1–P4 代码迁移已完成；P5 合成、真实旧 ERP 快照及真实 `data_handle` 隔离克隆演练已通过，生产切换未执行
>
> 本文只定义和实现代码边界，不授权生产 schema、历史数据迁移、发布或远端业务操作。
...
`data_handle` 执行。Long/UUID crosswalk 与兼容业务表已作为 0035–0037 迁移代码落地但尚未执行；兼容
...
当前阶段仅迁入代码并运行模拟测试，不对任何线上盘口发出这些请求。

## 5. 激活前置条件

兼容服务只有在以下条件全部满足后才允许通过生产 Compose 激活；服务定义可以预先进入
版本库供构建和配置校验，但不得据此启动或切流：
