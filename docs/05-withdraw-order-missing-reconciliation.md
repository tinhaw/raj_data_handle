...
`need_send_orders` 中包含 6587 条数据行：6174 条已成功、387 条已取消、26 条已失败。
`Sheet1` 中的 6174 条全部为已成功，并且按“系统订单号”验证为
`need_send_orders` 的完整子集，没有独有订单。

由于业务已确认所有平台状态都参与比对，初始模板默认只读取 `need_send_orders`。
`Sheet1` 不与其合并，避免 6174 条成功订单重复计算；它只用于结构校验或用户预览。
...
远端响应中包含银行账户、姓名、手机号和 IP 等敏感字段。本模块的 Normalizer 只能
提取订单匹配必需字段，禁止把完整 `info` 对象和无关 PII 发布到标准化层、页面或导出。
...
按渠道分别抓取远端提现订单，合并后保留每条订单的渠道代码和名称。
