...
- 默认预设 `source_id` 为 `rajwin`、`rajluck`，管理员可新增或删除使用相同接口契约的来源；
...
秒级刷新周期到达时，统一按 `Asia/Kolkata` 计算今天全天，或昨天全天＋今天全天的查询范围，
并对每个已启用盘口依次请求两次 `withdraw_order_summary`。只投影并返回两个状态的计数、查询时间和盘口级成功/失败状态；
