...
RajWin / RajLuck 只读数据分析与订单遗漏比对系统，并处于 ERP 台账、导入和兑换码模块
的代码迁入阶段。当前仓库已包含充值遗漏比对模块的 MVP 闭环：支付文件上传与模板识别、
用户确认盘口/渠道/时间口径、异步只读拉取、逐单精确复查、结果聚合、图表展示、共享批次、
站内通知和 CSV/Excel 导出。
...
ERP 的菜单、路由和领域边界已开始整理；本地台账、导入和兑换码能力的生产发布与数据库
结构迁移仍须单独确认目标环境、备份/回退方案和执行窗口。远端账号将以现有数据分析的“数据源与盘口”配置为基础升级为统一体系：
同一账号可按明确能力服务于分析或 ERP，权限仍分别校验。完整约定见
[ERP 域边界与迁移约定](docs/erp-domain-boundary-and-migration.md)。
